...
public class RtpVideoRenderer extends IMediaRenderer.Stub {
    ...
    public RtpVideoRenderer(String uri, OnConnectListener listener, SetUpParams params) throws Exception {
    ...
    public void open() {
        ...
        try {
        	...
        	Log.d("xxx", " open Exception e 1 = "+e);
            if (logger.isActivated()) {
...
//			Log.d("xxx", "writeSample --- sample.getData()[0] = " + sample.getData()[0]);
//			Log.d("xxx", "writeSample --- sample.getData()[1] = " + sample.getData()[1]);
//			Log.d("xxx", "writeSample --- sample.getData()[2] = " + sample.getData()[2]);
			if (sample.getData()[0] == 0 && sample.getData()[1] == 0
...
//					Log.d("xxx", "stop client");
			...
			if (sample.getData().length> 6 && sample.getData()[2] == -1 && sample.getData()[3] == -10 && sample.getData()[4] == -100) {
				Log.d("xxx", "writeSample result --- sample.getData()[5] = " + sample.getData()[5]);
				int intCount = (sample.getData().length - 6) / 4;
				if(intCount>0){
					int[] result = new int[intCount + 1];
					result[0] = sample.getData()[5];
					for (int i = 0; i < intCount; i++){
						byte[] tempByte = new byte[4];
						for (int j = 0; j < 4; j++){
							tempByte[j] = sample.getData()[6*(i+1)+j];
						}
			...
			if (selectedVideoCodec.getCodecName().equalsIgnoreCase(H264Config.CODEC_NAME)) {
				if (NativeH264Decoder.DecodeAndConvert(sample.getData(), decodedFrame) == 1) {
					...
					Log.d("xxx", "writeSample --- 264  sample.getData() length = "+sample.getData().length
							+ " decodedFrame.length = "+decodedFrame.length);
					if (surface != null) {

...
public class RtspControl implements RequestListener {
	...
	public RtspControl(String uri, SetUpParams params) {	
		...
		try {
			...
			this.params = params;
			Log.d("RtspViewer", " RtspControl uri = "+uri);
	...
	public void onError(RtspClient client, Throwable error) {
		...
		Log.d("xxx", "onError : client = "+client +"error : "+error);
...
//			this.client.teardown();
		...
		Log.d("xxx", "onFailure : client = "+client +"error : "+cause + "request : "+request);
		this.state = RtspConstants.UNDEFINED;
		this.connected = false;
	...
	public void onSuccess(RtspClient client, Request request, Response response) {
		Log.d("xxx", "onSuccess : client = "+client +"response : "+response + "request : "+request);
		try {
			...
			if ((this.client != null) && (response.getStatusCode() == 200)) {
				...
				if (method == Request.Method.OPTIONS) {
					...
					if (video != null) {
						...
						// send SETUP request
						this.client.setup(this.uri, this.port, this.resource,this.params);
						
					}

				} else if (method == Request.Method.SETUP) {
					Log.d("xxx"," method = Request.Method.SETUP");
					// set state to READY
					this.state = RtspConstants.READY;

				} else if (method == Request.Method.PLAY) {
					Log.d("xxx"," method = Request.Method.PLAY");
					// set state to PLAYING
					this.state = RtspConstants.PLAYING;

				} else if (method == Request.Method.PAUSE) {
					Log.d("xxx"," method = Request.Method.PAUSE");
					// set state to READY
					this.state = RtspConstants.READY;

				} else if (method == Request.Method.TEARDOWN) {

					Log.d("xxx"," method = Request.Method.TEARDOWN");
					this.connected = false;

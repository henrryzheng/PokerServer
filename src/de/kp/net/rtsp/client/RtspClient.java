...
public class RtspClient implements TransportListener {
	...
	public void options(String uri, URI endpoint) {
		...
		try {
			...
			Log.d("xxx", " RtspControl uri = "+uri + "failed : "+e);
	...
	public void setup(URI uri, int localPort, String resource, SetUpParams params) {
		...
		try {
			...
			String portParam = "client_port=" + localPort + "-" + (1 + localPort);
			String finalURI = uri.toString();
			String gameType = "gameType" + "-" + params.getGameType();
			String peopleNum = "peopleNum" + "-" + params.getPeopleNum();
			String brocastType = "brocastType" + "-" + params.getBroadcastType();
			String playType = "playType" + "-" + params.getPlayType();
			String ev = "ev" + "-" + params.getEV();
			
			if ((resource != null) && (resource.equals("*") == false))
				finalURI += '/' + resource;
			
			send(getSetup(finalURI, localPort, new TransportHeader(LowerTransport.DEFAULT, "unicast", portParam,"setupParams",gameType,peopleNum,brocastType,playType,ev,"end"), session));
		...
		while(messageBuffer.getLength() > 0)
			try
			{
				...
				if(message instanceof RtspRequest) {
					Log.d("xxx", "dataReceived :"+message.getCSeq().getValue());
	...
	private void send(Message message, URI endpoint) throws Exception
	{
		...
		Log.d("xxx", " send message.getCSeq().getValue() = "+message.getCSeq().getValue());
		if(message instanceof RtspRequest)
		{
			Log.d("xxx", " send request message.getCSeq().getValue() = "+message.getCSeq().getValue());
			...
			Log.d("xxx", " send not request message.getCSeq().getValue() = "+message.getCSeq().getValue());
			transport.sendMessage(message);
		}
	}

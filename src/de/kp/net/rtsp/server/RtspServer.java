...
public class RtspServer implements Runnable {
	...
	public void run() {
	    ...
	    while (this.stopped == false) {
			...
			try {
				...
				e.printStackTrace();
				Log.d("xxx", " serverSocket.accept error : "+e);
			}
	...
	public void stop() {
		...
		try {
			...
			Log.d("xxx", " serverSocket close ¡êo "+e);
			// nothing todo
		}
	    	...
	    	try {
	    		...
	    		while (setup == false) {
		    		...
		    		if (requestType == RtspConstants.DESCRIBE) {
		    			PeerCamera.getInstance().setSendVideoFlag(true);
		    			RtspVideoRecorder.getInstance().setSendVideoFlag(true);
		    		}

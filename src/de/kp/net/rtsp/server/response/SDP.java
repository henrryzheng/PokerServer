...
//			buf.append("a=fmtp:98 packetization-mode=1;profile-level-id=420020;sprop-parameter-sets=J0IAINoFB8Q=,KM48gA==;" + RtspResponse.CRLF); // 320x240 10fps
		...
		} else if (encoder.equals(VideoEncoder.Mediacodec_ENCODER)) {
			// cross encoder properties
			sb.append("m=video " + clientVideoPort + RtspConstants.SEP + "RTP/AVP " + RtspConstants.RTP_MediaCodec_PAYLOADTYPE + RtspResponse.CRLF);	
			// set to H263-2000
			sb.append("a=rtpmap:" + RtspConstants.RTP_MediaCodec_PAYLOADTYPE + RtspConstants.SEP + RtspConstants.MediaCodec + RtspResponse.CRLF);

			// additional information for android video view, due to extended checking mechanism
			sb.append("a=framesize:" + RtspConstants.RTP_MediaCodec_PAYLOADTYPE + RtspConstants.SEP + RtspConstants.WIDTH + "-" + RtspConstants.HEIGHT + RtspResponse.CRLF);
		}

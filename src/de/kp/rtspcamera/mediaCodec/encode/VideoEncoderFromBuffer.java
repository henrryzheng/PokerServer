...
//	private String mPath = MyApplication.getAppContext().getFilesDir().getAbsolutePath() + "/h264/";
	...
	private static final int BIT_RATE = PeerCamera.IMAGE_HEIGHT * PeerCamera.IMAGE_WIDTH * 3 * 8 * FRAME_RATE / COMPRESS_RATIO; // bit rate CameraWrapper.
	private int mWidth;
	private int mHeight;
	private MediaCodec mMediaCodec;
//	private MediaMuxer mMuxer;   //鐏忓棝鐓剁憴鍡涱暥閸樺缂夐幋鎭梡4 閺嶇厧绱℃穱婵嗙摠閸掔増婀伴崷甯礉妤犲矁鐦塵ediaCodec閺勵垰鎯佸锝団�
...
//	private boolean mMuxerStarted;
	...
	public VideoEncoderFromBuffer(int width, int height) throws IOException {
		...
		mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, /*BIT_RATE*/125000 /*500000*/);
		mediaFormat.setInteger(MediaFormat.KEY_FRAME_RATE, FRAME_RATE);
//		mediaFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT, /*mColorFormat*/ MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar);
		mediaFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT, /*mColorFormat*/ MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar);
...
//				+ this.mHeight + ".mp4";
//        String fileName = MyApplication.getAppContext().getFilesDir().getAbsolutePath() + "/h264/" + "test.mp4" ;
...
//	}
	...
	public void onFrame(byte[] data, int offset, int length, int flag) {
		ByteBuffer[] inputBuffers = mMediaCodec.getInputBuffers();
		ByteBuffer[] outputBuffers = mMediaCodec.getOutputBuffers();
	//	byte[] buf = swapYV12toI420(data, offset, length);
	//	NV21toI420SemiPlanar(data, mFrameData, this.mWidth, this.mHeight);
		swapYV12toI420(data, mFrameData, this.mWidth, this.mHeight);
		int inputBufferIndex = mMediaCodec.dequeueInputBuffer(-1);
		if (inputBufferIndex >= 0) {
			ByteBuffer inputBuffer = inputBuffers[inputBufferIndex];
			inputBuffer.clear();
			inputBuffer.put(mFrameData, offset, length);
		...
		while (outputBufferIndex >= 0) {
			...
			{
				Log.d("encoder","------------------------0 outData.length = "+outData.length);
				PeerCamera.getInstance().onFrame(outData, 3, outData.length - 3, bufferInfo.flags);
			}
			else
			{
				Log.d("encoder","------------------------1 outData.length = " + outData.length);
				outData[0] = 0;
				outData[1] = 0;
				outData[2] = 1;
				PeerCamera.getInstance().onFrame(outData, 0, outData.length, bufferInfo.flags);
			}

			//PeerCamera.getInstance().onFrame(new byte[outputBuffer.remaining()], 0, length, flag);

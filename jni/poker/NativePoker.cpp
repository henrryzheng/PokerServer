...
int newestCard;
int numOfPeople;
volatile unsigned int pokerResult[55];
short   pokerResultLen      	= 0;
...
bool isPokerDealCompleted() {
	if(Consequence->para_num<2){
		return false;
	}
	else{
		for(int i=0;i<Consequence->para_num;i++){
			if (Consequence->card_para_num[i] != 3){
				return false;
			}
			for(int j = 0; j< Consequence->card_para_num[i]; j++){
				if(Consequence->card_para[i][j]>51||Consequence->card_para[i][j]<0){
					return false;
				}
			}
		}
...
JNIEXPORT void JNICALL Java_de_kp_rtspcamera_poker_NativePoker_DecodePicture
  (JNIEnv *env, jclass obj, jbyteArray indata, jint img_w, jint img_h, jint customerPeopleNum){
    if(change == 1)
    {
        init();
        change = 0;
...
//            gwCLen = 0;
            if(customerPeopleNum == 0){
            	numOfPeople = Consequence->para_num;
            }
            else{
            	numOfPeople = customerPeopleNum;
            }
...
//                  }
			...
			if(preNewst_card != newestCard){
			   preNewst_card = newestCard;
			   if(newestCard<55 && newestCard >= 0){
				   LOGD("callback newestCard = %d \n",newestCard);
				   jclass cls = (env)->FindClass(PKG_CLASSNAME);
				   jmethodID callbackDanPai = (env)->GetStaticMethodID(cls, "onCallbackPlayDanPai",  "(I)V");
				   (env)->CallStaticVoidMethod(cls, callbackDanPai,newestCard);
				   if(newestCard<52 && newestCard >= 0){
					   pokerResult[pokerResultLen] = newestCard;
					   pokerResultLen += 1;
					   if(pokerResultLen>54){
						   pokerResultLen = 54;
					   }
				   }
			   }
			...
			if(customerPeopleNum == 0){
				if(isPokerDealCompleted() == true){
					LOGD("isPokerDealCompleted = true");
					jobjectArray pokerResults;
					jclass intClass = env->FindClass("[I");
					pokerResults = (env)->NewObjectArray(numOfPeople,intClass,NULL);
	...
	//					  }
					...
					do{
					   jclass cls = (env)->FindClass(PKG_CLASSNAME);
					   jmethodID callbackDoAlgo = (env)->GetStaticMethodID(cls, "onCallbackDoAlgo",  "(I[[I)V");
					   (env)->CallStaticVoidMethod(cls, callbackDoAlgo,numOfPeople,pokerResults);
					}while (0);
				...
				if(pokerResultLen == customerPeopleNum*3){
					LOGD("pokerResultLen = %d done!",pokerResultLen);
					int (*pokerResultTemp)[3] = new int[customerPeopleNum][3];
					for(int i=0; i<customerPeopleNum; i++){
						for(int j = 0; j<3; j++){
							pokerResultTemp[i][j] = pokerResult[i+j*customerPeopleNum];
						}
					}
					jobjectArray pokerResults;
					//
					jclass intClass = env->FindClass("[I");
					pokerResults = (env)->NewObjectArray(customerPeopleNum,intClass,NULL);

					for(int i=0;i<customerPeopleNum;i++){

					  jintArray intdata = env->NewIntArray(3);
					  env->SetIntArrayRegion(intdata,0,3,(jint*)& pokerResultTemp[i]);
					  env->SetObjectArrayElement(pokerResults,i,intdata);
					  env->DeleteLocalRef(intdata);
	//					  for(int j = 0; j < Consequence->card_para_num[i]; j++) {
	//						  Itoa3(Consequence->card_para[i][j]);
	//					  }
					...
					delete[] pokerResultTemp;

				}
				else{
					LOGD("pokerResultLen = %d",pokerResultLen);
				}
			}
        }
        else
